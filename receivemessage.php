  ...
  <?if(!empty($files)){?>
	<div class="data">
	  <?foreach($files as $file) {
        ...
        exec($config_values['SOX']." ".str_replace(" ","\ ",$file[path]).' -t raw -r 8000 -s -c 1 /var/tmp/uploads/x-'.$hash.'.sln');
        exec($config_values['SOX']." ".str_replace(" ","\ ",$file[path]).' -r 8000 -s -c 1 /var/tmp/uploads/x-'.$hash.'.wav');
        exec($config_values['SOX']." ".str_replace(" ","\ ",$file[path]).' -r 8000 -s -c 1 ./uploads/x-'.$hash.'.wav');
        ?>
