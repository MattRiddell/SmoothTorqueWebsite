...
if (!function_exists('create_missing_tables') ) {
    function create_missing_tables($db_host,$db_user,$db_pass) {
        ...
        if (!in_array('numbers_warning', $field_array)) {
            $result = mysql_query('ALTER TABLE web_config ADD numbers_warning varchar(250)') or die(mysql_error());
            $result = mysql_query('UPDATE web_config SET numbers_warning = "Number Exhaustion Warning Emails"') or die(mysql_error());            
            $sql = "INSERT INTO log (timestamp, username, activity) VALUES (NOW(), '$_POST[user]', 'Added web_config numbers_warning field')";
        ...
        $result = mysql_query("SHOW COLUMNS FROM SineDialer.cdr") or die(mysql_error());
        $columns = mysql_num_rows($result) or die(mysql_error());
        unset($field_array);
        for ($i = 0; $i < $columns; $i++) {
            $field_array[] = mysql_result($result, $i, "Field");
